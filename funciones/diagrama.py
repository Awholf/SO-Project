...
def generar_diagrama_gantt(planificacion):
    ...
    for tarea in planificacion:
        ax.broken_barh(
            [(tarea["inicio"], tarea["fin"] - tarea["inicio"])],  # Duración
            (tarea["id"] * 10, 9),  # Posición del proceso en el eje Y
            facecolors=('tab:blue')
        )
    ...
    ax.set_ylim(0, (len(planificacion) + 1) * 10)  # Altura del gráfico
    ax.set_xlim(0, max(t["fin"] for t in planificacion) + 1)  # Ancho del gráfico
